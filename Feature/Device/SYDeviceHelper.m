...
+ (void)setDriverToken:(NSData *)deviceToken{
...
+ (NSString *)getAppCertID{
    ...
    NSString *bundleid = [NSString stringWithFormat:@"%@.%@",[self getBundleId],KEYCHAIN_COMPANY_FLAG];
    NSString *md5Bundleid = [self MD5ForLower32Bate:bundleid];
    NSString *md5service = [self MD5ForLower32Bate:KEYCHAIN_SERVICE_NAME];
...
- (NSString *)getAppVersion
{
    ...
    self.appVersion = infoDic[@"CFBundleShortVersionString"];
    ...
    self.appVersion=[NSString stringWithFormat:@"%@.%@",self.appVersion,resVersion];
